...
const generateFormEightPDF = (data, outputFilePath, callback) => {
  try {
    ...
    doc.text(
      `The last issued Peer Review Certificate of our Firm was valid till ${data.lastIssuedCertificateDate}. We request you to kindly extend the validity of the Certificate from ${data.extensionFromDate} to ${data.extensionToDate}.`,
      { align: "justify", width: contentWidth }
    );
    ...
    doc.text("Enclosure: Medical Certificate (if applicable)", {
      align: "left",
      width: contentWidth,
    });
